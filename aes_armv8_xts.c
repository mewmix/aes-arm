...
static void aes256_encrypt_block_soft(const uint8_t (*rk)[16],
                                      const uint8_t in[16],
                                      uint8_t out[16]);
#endif
...
#if defined(__aarch64__) && defined(__ARM_FEATURE_CRYPTO)
    aes256_encrypt_block_armv8_asm(rk, in, out);
#else
...
#if defined(__aarch64__) && defined(__ARM_FEATURE_CRYPTO)
    aes256_decrypt_block_armv8_asm(rk, in, out);
#else
...
static __attribute__((unused)) void aes256_encrypt_block_soft(const uint8_t (*rk)[16],
                                                              const uint8_t in[16],
                                                              uint8_t out[16])
{
    uint8_t state[16];
    ...
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, rk[AES256_ROUNDS]);
                            ...
                            int encrypt)
{
    ...
    aes256_encrypt_block_armv8(tweak_keys->enc, iv, tweak);
    ...
    for (size_t off = 0; off < len; off += 16) {
        ...
        if (encrypt) {
            aes256_encrypt_block_armv8(data_keys->enc, tmp, buf);
        } else {
            aes256_decrypt_block_armv8(data_keys->dec, tmp, buf);
        }
